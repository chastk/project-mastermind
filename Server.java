...
class Worker implements Runnable {
	...
	public Worker(Account[] allAccounts, String trans) {
		...
		transaction = trans;
		toRead = new boolean[numLetters];
		toWrite = new boolean[numLetters];
		rwReset();
	}
	
	private void rwReset(){
	...
	private int parseAccountOrNum(String name) {
		...
		if (name.charAt(0) >= '0' && name.charAt(0) <= '9') {
			...
			// If we're parsing an account here, it's to read the value from it
			// So we'll need to open it.
			toRead[accountNum] = true;
		}
		return rtn;
	}
		...
		do {
			...
			try {
				...
				for(int j = 0; j < numLetters; j++){
					// (something can be open for both)
					if(toRead[j])
				...
				// ... close when finished ...
				for(int j = 0; j < numLetters; j++){
					if(toRead[j] || toWrite[j])
						accounts[j].close();
				}
				// ... and prepare for next loop iteration.
				rwReset();
				i++;
			} catch (TransactionAbortException e) {
				// If we don't perform the transaction, reset the locks,
				// and DON'T increment to the next command.
				System.out.println("Someone else has a lock I need! Retrying....");
				for(int j = 0; j < numLetters; j++)
					accounts[j].close();
				rwReset();
                continue;
